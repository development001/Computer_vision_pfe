...
let currentLineCoords = null; // [x1, y1, x2, y2]
let isDrawing = false;
let startPoint = null;
...
async function openLineModal(cameraId) {
    ...
    // Resize canvas to fit modal but keep aspect ratio
...
async function fetchCameras(){
  try {
    ...
    // Check if there's a pre-selected camera from URL
...
async function stopJob(job_id){
  ...
  try {
    ...
    // If we were viewing this job, close the viewer
